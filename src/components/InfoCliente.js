...
const InfoCliente = (props) => {
  ...
  const [clientContactInformation, setClientContactInformation] =
    useState(null);
  ...
  useEffect(() => {
    console.log("Contact Event - Contact ID from useEffect:", {
      clientContactId,
    });
    //If the contact ID is not null, fetch the client information
    if (clientContactId !== null) {
      console.log(
        "Contact Event - Contact ID not null, attempting to fetch client information"
      );
      fetchCustomerProfile(clientPhoneNumber);
    } else {
      console.log(
        "Contact Event - Contact ID is null, not fetching client information"
      );
  ...
  useEffect(() => {
    ...
    if (clientContactInformation !== null) {
      console.log("Contact Event - Contact information from useEffect:", {
  ...
  const fetchCustomerProfile = async (phone) => {
    ...
    try {
      ...
      if (response.Items && response.Items.length > 0) {
        ...
        console.log(
          "Contact Event - Successfully fetched contact info:",
          contact
        );
      ...
      console.error(
        "Contact Event - Error fetching contact information:",
        error
      );
    }
  };
